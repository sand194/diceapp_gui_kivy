...
# public metehods:
    ...
    def __save(self): # private method
        if len(self._memory) > 1:
            ...
            with open(r'D:/python_programs/diceapp_oop/roll_history_files/history_file_' + f_name + '.txt', 'w') as f_path:
    ...
    def _quit(self): # protected method
        ...
        if len(self._memory) > 1: # if rolls were made the history folder will open
            ...
            time.sleep(0.75)
            os.startfile('D:/python_programs/diceapp_oop/roll_history_files') # opening the folder where history files are located
